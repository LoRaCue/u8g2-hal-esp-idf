...
static i2c_master_dev_handle_t i2c_dev = NULL;  // I2C device handle.
  ...
  switch (msg) {
    ...
    case U8X8_MSG_BYTE_INIT: {
      ...
      ESP_LOGI(TAG, "Initializing I2C bus: SDA=%d, SCL=%d, freq=%d Hz",
               u8g2_esp32_hal.bus.i2c.sda, u8g2_esp32_hal.bus.i2c.scl, I2C_MASTER_FREQ_HZ);
      ESP_ERROR_CHECK(i2c_new_master_bus(&bus_config, &i2c_bus));

      // Add I2C device (will be configured with address on first transfer)
      break;
    }
    ...
    case U8X8_MSG_BYTE_START_TRANSFER: {
      uint8_t i2c_address = u8x8_GetI2CAddress(u8x8) >> 1;  // u8x8 uses 8-bit address
    ...
    case U8X8_MSG_BYTE_END_TRANSFER: {
      ...
      if (i2c_buffer_len > 0) {
        ESP_ERROR_CHECK(i2c_master_transmit(i2c_dev, i2c_buffer, i2c_buffer_len, I2C_TIMEOUT_MS));
      }
      break;
    }
  }
